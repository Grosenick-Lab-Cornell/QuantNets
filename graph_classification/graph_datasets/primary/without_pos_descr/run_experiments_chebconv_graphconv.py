...
from gnn.architectures import ChebConvNet, GraphConvNet
    ...
    for dataset_group in dataset_groups:
        for dataset_split in datasets[dataset_group]: # loop over datasets
            ...
            for selected_dataset, selected_dataset_config in dataset_mapping.items():
                ...
                experiment_name = f"BATCH-RESULTS-ALL-DATASETS-{selected_dataset.capitalize()}_ChebConv_GraphConv_Summary"
                ...
                cheb_K         = selected_dataset_config.get("cheb_K", 1)
                cheb_out_sf    = selected_dataset_config.get("cheb_out_sf", 1)
                cheb_hidden_sf = selected_dataset_config.get("cheb_hidden_sf", 1)

                graph_out_sf    = selected_dataset_config.get("graph_out_sf", 1)
                graph_hidden_sf = selected_dataset_config.get("graph_hidden_sf", 1)
                ...
                for i, lr in enumerate(lrs[dataset_group]): # loop over learning rates
                    ...
                    for run in range(num_runs): # loop over num runs
                        ...
                        chebconv_model = ChebConvNet(out_dim=out_dim, 
                                                    input_features=in_channels, 
                                                    output_channels=out_channels, 
                                                    layers_num=layers_num, 
                                                    model_dim=hidden_channels,
                                                    hidden_sf=cheb_hidden_sf, 
                                                    out_sf=cheb_out_sf, 
                                                    K=cheb_K)
                        
                        graphconv_model = GraphConvNet(out_dim=out_dim, 
                                                    input_features=in_channels,
                                                    output_channels=out_channels,
                                                    layers_num=layers_num,
                                                    model_dim=hidden_channels,
                                                    hidden_sf=graph_hidden_sf, 
                                                    out_sf=graph_out_sf)
                        ...
                        experiment = Experiment(sgcn_model = chebconv_model,
                                                qgcn_model = graphconv_model,
                                                cnn_model = None,
