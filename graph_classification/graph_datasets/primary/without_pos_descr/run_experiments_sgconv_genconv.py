...
from gnn.architectures import SGConvNet, GENConvNet
    ...
    for dataset_group in dataset_groups:
        for dataset_split in datasets[dataset_group]: # loop over datasets
            ...
            for selected_dataset, selected_dataset_config in dataset_mapping.items():
                ...
                experiment_name = f"BATCH-RESULTS-ALL-DATASETS-{selected_dataset.capitalize()}_SGConv_GENConv_Summary"
                ...
                sg_K         = selected_dataset_config.get("sg_K", 1)
                sg_out_sf    = selected_dataset_config.get("sg_out_sf", 1)
                sg_hidden_sf = selected_dataset_config.get("sg_hidden_sf", 1)

                gen_out_sf    = selected_dataset_config.get("gen_out_sf", 1)
                gen_hidden_sf = selected_dataset_config.get("gen_hidden_sf", 1)
                ...
                for i, lr in enumerate(lrs[dataset_group]): # loop over learning rates
                    ...
                    for run in range(num_runs): # loop over num runs
                        ...
                        sgconv_model = SGConvNet(out_dim=out_dim, 
                                                input_features=in_channels, 
                                                output_channels=out_channels, 
                                                layers_num=layers_num, 
                                                model_dim=hidden_channels,
                                                hidden_sf=sg_hidden_sf, 
                                                out_sf=sg_out_sf, 
                                                K=sg_K)
                        
                        genconv_model = GENConvNet(out_dim=out_dim, 
                                                input_features=in_channels,
                                                output_channels=out_channels,
                                                layers_num=layers_num,
                                                model_dim=hidden_channels,
                                                hidden_sf=gen_hidden_sf, 
                                                out_sf=gen_out_sf)
                        ...
                        experiment = Experiment(sgcn_model = sgconv_model,
                                                qgcn_model = genconv_model,
                                                cnn_model = None,
