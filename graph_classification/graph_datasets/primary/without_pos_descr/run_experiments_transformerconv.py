...
from gnn.architectures import TransformerConvNet
    ...
    for dataset_group in dataset_groups:
        for dataset_split in datasets[dataset_group]: # loop over datasets
            ...
            for selected_dataset, selected_dataset_config in dataset_mapping.items():
                ...
                experiment_name = f"BATCH-RESULTS-ALL-DATASETS-{selected_dataset.capitalize()}_TransformerConv_Summary"
                ...
                transf_hidden_heads = selected_dataset_config.get("transf_hidden_heads", 1)
                transf_out_sf       = selected_dataset_config.get("transf_out_sf", 1)
                transf_hidden_sf    = selected_dataset_config.get("transf_hidden_sf", 1)
                ...
                for i, lr in enumerate(lrs[dataset_group]): # loop over learning rates
                    ...
                    for run in range(num_runs): # loop over num runs
                        ...
                        generalconv_model = None
                        transfconv_model  = TransformerConvNet(out_dim=out_dim, 
                                                               input_features=in_channels,
                                                               output_channels=out_channels,
                                                               layers_num=layers_num,
                                                               model_dim=hidden_channels,
                                                               hidden_sf=transf_hidden_sf, 
                                                               out_sf=transf_out_sf,
                                                               hidden_heads=transf_hidden_heads)
