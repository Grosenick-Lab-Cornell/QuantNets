...
from sgcn.src.architectures import QGRN_Equiv_SGCN
    ...
    for dataset_group in dataset_groups:
        for dataset_split in datasets[dataset_group]: # loop over datasets
            ...
            for selected_dataset, selected_dataset_config in dataset_mapping.items():
                ...
                experiment_name = f"BATCH-RESULTS-ALL-DATASETS-{selected_dataset.capitalize()}_SGCN_QGRN_Summary"
                ...
                sgcn_out_sf      = selected_dataset_config.get("sgcn_out_sf", 1)
                sgcn_hidden_sf   = selected_dataset_config.get("sgcn_hidden_sf", 1)
                sgcn_hidden_size = selected_dataset_config.get("sgcn_hidden_size", 1)
                ...
                for i, lr in enumerate(lrs[dataset_group]): # loop over learning rates
                    ...
                    for run in range(num_runs): # loop over num runs
                        ...
                        sgcn_model = QGRN_Equiv_SGCN(out_dim=out_dim,
                                                    input_features=in_channels,
                                                    layers_num=layers_num,
                                                    model_dim=hidden_channels,
                                                    out_channels_1=out_channels,
                                                    dim_coor=pos_descr_dim,
                                                    hidden_sf=sgcn_hidden_sf, 
                                                    out_sf=sgcn_out_sf, 
                                                    hidden_size=sgcn_hidden_size)
